...
- **Budget and resourcing must include AI policy — and AI disclosure is now deadline-bound, not discretionary (hardened v2.2).** Specify which AI tools are sanctioned for which assets, whether output requires human review before publication, and the disclosure + provenance obligation. As of mid-2026 this is a hard line, not a brand-judgment call: EU AI Act Article 50 deployer disclosure duties and machine-readable marking become enforceable Aug 2, 2026 (marking extension Dec 2, 2026; fines up to €15M / 3% of global turnover; extraterritorial, so any EU-reaching asset is in scope), the C2PA / SynthID provenance stack is the marking mechanism (and its metadata is fragile through social pipelines, so pair it with model watermarking + a visible label), California SB 942 / AB 853 run a parallel US track, and FTC endorsement/deception rules already reach undisclosed AI in ads. The brief should record, per deliverable: the disclosure method and wording, whether C2PA/watermark provenance marking is required at export, and the named person who signs off that the asset is disclosed, marked, and claim-verified (the internal analog of the Cannes Lions CEO/CMO entry sign-off). Operative detail lives in `knowledge-base/regulations/ai-content-disclosure.md`. Briefs that omit this surface conflict at publication, not at kickoff — and now risk a regulatory penalty rather than just an awkward edit.
